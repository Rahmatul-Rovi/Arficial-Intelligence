...
# ------------------------------------------------------------
# HOW TO RUN:
# 1. colab.research.google.com এ যাও
# 2. New Notebook খোলো
# 3. এই পুরো code টা copy করে paste করো
# 4. Runtime → Change runtime type → T4 GPU select করো
# 5. Runtime → Run all করো
# ============================================================
...
# ─────────────────────────────────────────────
# Step 1: kaggle.com → Account → API → "Create New Token"
#         → kaggle.json ফাইল download হবে
# Step 2: নিচের code uncomment করে run করো

from google.colab import files
print("kaggle.json ফাইল upload করো:")
files.upload()
!mkdir -p ~/.kaggle
...
# ─────────────────────────────────────────────
# Dataset path (Kaggle download করার পর এটা set করো)
DATASET_PATH = '/content/plantvillage/plantvillage dataset/color'

IMG_SIZE   = 128   # 128x128 pixels (Colab free tier এ fast চলবে)
BATCH_SIZE = 32
EPOCHS     = 20
...
train_datagen = ImageDataGenerator(
    ...
    rotation_range    = 20,          # ±20 ডিগ্রি rotate
    horizontal_flip   = True,        # বাম-ডান উল্টানো
    zoom_range        = 0.2,         # zoom
    width_shift_range = 0.1,         # বাম-ডান shift
    height_shift_range= 0.1,         # উপর-নিচ shift
...
plt.show()
print("Sample images দেখানো হয়েছে!")

# ─────────────────────────────────────────────
# CELL 6: CNN Model তৈরি
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# CELL 8: Model Train করো
# ─────────────────────────────────────────────
print("\n" + "="*55)
print("  Model Training শুরু হচ্ছে...")
print("="*55 + "\n")
...
# ─────────────────────────────────────────────
print("\nConfusion Matrix তৈরি হচ্ছে...")
print("(এটা কিছুটা সময় নেবে...)\n")
...
# Confusion Matrix Plot (top 10 classes for readability)
# সব 38 class একসাথে দেখা কঠিন, তাই top classes নিচ্ছি
...
# ─────────────────────────────────────────────
# CELL 12: Model Save করো
# ─────────────────────────────────────────────
model.save('plant_disease_cnn_final.h5')
print("✅ Model save হয়েছে: plant_disease_cnn_final.h5")

# ─────────────────────────────────────────────
# CELL 13: নতুন Image দিয়ে Predict করো
# ─────────────────────────────────────────────
def predict_single_image(image_path):
    """
    একটা leaf image দাও, disease বলে দেবে!
...
# ব্যবহার করতে:
# predict_single_image('/content/your_leaf_photo.jpg')
